...
from src.database import HHAPI, Database
...
class TestHHAPI:
    ...
    def test_get_employer_vacancies_no_items(self, mock_get, hh_api):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"items": [], "pages": 0, "page": 0}
        mock_get.return_value = mock_response
...
class TestDatabase:
    ...
    def test_create_database_existing_db(self, mock_connect):
        ...
        create_db_calls = [
            call
            for call in mock_cursor1.execute.call_args_list
    ...
    def test_insert_employer_data_with_none(self, mock_sleep, mock_hh_api, mock_db):
        ...
        mock_api_instance.get_employer_info.side_effect = [
            ...
            None,  # Вторая компания возвращает None
            {
                "name": "Сбер",
                "description": "Test",
                "site_url": "https://sber.ru",
                "open_vacancies": 3,
            },
        ] + [
            None
        ] * 7  # Остальные компании тоже возвращают None
